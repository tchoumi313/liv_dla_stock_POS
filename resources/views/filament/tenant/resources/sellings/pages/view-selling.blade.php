        ...
        <div class="flex">
            <div class="w-full print:w-1/3 md:w-1/3 px-2">
                <div>
                    ...
                    <div class="details">
                        <ul class="my-1">
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Code')</span><span>#{{ $record->code }}</span></li>
                            @if (About::first() && About::first()->business_type == 'fnb')
                                <li class="flex justify-between text-secondary text-sm mb-1"><span
                                        class="font-semibold">@lang('Table')</span><span>{{ $record->table?->number ?? 'N/A' }}</span>
                                </li>
                            @endif
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Cashier')</span><span>{{ $record->user->name }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Date')</span><span>{{ now()->parse($record->date)->setTimezone(Profile::get()->timezone ?? 'UTC')->format('d F Y H:i') }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Payment method')</span><span>{{ $record->paymentMethod->name }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Voucher')</span><span>{{ $record->voucher ?? 'N/A' }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="w-full print:w-1/3 md:w-1/3 px-2">
                <div>
                    ...
                    <div class="details">
                        <ul class="my-1">
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Name')</span><span>{{ $record->member?->name ?? 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Code')</span><span>{{ $record->member?->code ?? 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Joined date')</span><span>{{ $record->member?->joined_date? now()->parse($record->member?->joined_date)->setTimezone(Profile::get()->timezone ?? 'UTC')->format('d F Y H:i'): 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Identity type')</span><span>{{ $record->member?->identity_type ?? 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Identity number')</span><span>{{ $record->member?->identity_number ?? 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Contact')</span><span>{{ $record->member?->email ?? 'N/A' }}</span>
                            </li>
                            <li class="flex justify-between text-secondary text-sm mb-1"><span
                                    class="font-semibold">@lang('Address')</span><span>{{ $record->member?->address ?? 'N/A' }}</span>
                            </li>
                            <!---->
                        </ul>
                    </div>
            ...
            <table class="table ns-table w-full">
                ...
                <tbody>
                    @foreach ($record->sellingDetails as $detail)
                        <tr>
                            <td class="p-2 border">
                                <h3 class="text-primary">{{ $detail->product->name }}</h3><span
                                    class="text-sm text-secondary"></span>
                            </td>
                            <td class="p-2 border text-center text-primary">
                                {{ Number::currency($detail->price, Setting::get('currency', 'XOF')) }}</td>
                            <td class="p-2 border text-center text-primary">{{ $detail->qty }}</td>
                            <td class="p-2 border text-center text-primary">
                                {{ Number::currency($detail->discount_price, Setting::get('currency', 'XOF')) }}</td>
                            <td class="p-2 border text-center text-primary">
                                {{ Number::currency($detail->total_price, Setting::get('currency', 'XOF')) }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="font-semibold">
                    <tr>
                        ...
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->total_price, Setting::get('currency', 'XOF')) }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border text-center text-primary" colspan="3"></td>
                        <td class="p-2 border text-primary text-left">@lang('Discount')</td>
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->total_discount_per_item + $record->discount_price, Setting::get('currency', 'XOF')) }}
                        </td>
                    </tr>
                    <!---->
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->tax_price, Setting::get('currency', 'XOF')) }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border text-center text-primary" colspan="3"></td>
                        <td class="p-2 border text-primary text-left">@lang('Total')</td>
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->grand_total_price, Setting::get('currency', 'XOF')) }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border text-center text-primary" colspan="3"></td>
                        <td class="p-2 border text-primary text-left">@lang('Payed money')</td>
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->payed_money, Setting::get('currency', 'XOF')) }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border text-center text-primary" colspan="3"></td>
                        <td class="p-2 border text-primary text-left">@lang('Money changes')</td>
                        <td class="p-2 border text-right text-primary">
                            {{ Number::currency($record->money_changes, Setting::get('currency', 'XOF')) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
            ...
            try {
                if (!printerData) {
                    ...
                    selling.selling_details.forEach(sellingDetail => {
                        ...
                        let text = moneyFormat(sellingDetail.price / sellingDetail.qty) + ' x ' +
                            sellingDetail.qty.toString();
                        printerAction.table([sellingDetail.product.name, moneyFormat(sellingDetail
                                .price / sellingDetail.qty) + ' x ' + sellingDetail.qty
                            .toString()
                        ])
